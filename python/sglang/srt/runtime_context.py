...
globals that were scattered across many files. Organized by subsystem:
  ...
  ctx.buffers  : lazily-materialized buffers (``get_buffer()`` allocates on first
                 access and caches; ``release()`` drops the cache).
...
class ParallelContext:
    ...
    local_attn_dp_size: Optional[int] = None
...
class BufferSpec:
    """Metadata for a lazily-materialized buffer — its shape/dtype/device, NOT the
    physical tensor (generated at get_buffer time, not pre-allocated). ``shape`` is
    passed straight to ``torch.zeros`` (an int for a 1-D buffer, or a tuple)."""
...
class BufferStore:
    """Lazy-materialize-at-get + cache + release. ``register`` records metadata
    only; the tensor is allocated on the FIRST ``get_buffer(name)`` and cached, so
    every later caller shares one object (data_ptr stable), equivalent to the old
    ``global ... if None`` init-once. ``release`` (folded into reset) drops the
    cache so a fresh engine / test re-allocates."""
    ...
    def release(self) -> None:
        # drop cached tensors; next get_buffer re-allocates. Wired only into reset.
        self._cache.clear()
